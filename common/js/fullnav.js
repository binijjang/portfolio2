...
var on_off=false;  //false(안오버)  true(오버)
   ...
   $('#headerArea').mouseleave(function(){
         ...
         if(scroll<smh-100 ){
            $(this).css('background','rgba(255,255,255,.9)').css('border-bottom','none');
   ...
   $(window).on('scroll',function(){//스크롤의 거리가 발생하면
         ...
         if(scroll>smh-100){//스크롤이 비주얼의 높이-header높이 까지 내리면
            $('#headerArea').css('background','#fff').css('box-shadow','0 1px 10px 1px rgba(0, 0, 0, 0.1)');
            // $('.dropdownmenu li a').css('color','#333');
         }else{//스크롤 내리기 전 디폴트(마우스올리지않음)
            if(on_off==false){
                  $('#headerArea').css('background','rgba(255,255,255,.9)').css('border-bottom','none');
               // $('.dropdownmenu li a').css('color','#fff');
            }
         }; 
   ...
   $('ul.dropdownmenu li.menu').hover(
      function() {
         $('.depth1',this).css('color','var(--font-color-main)');
      },function() {
...
$('.pop_onoff').toggle(function(e){
   e.preventDefault();
   $('.product_pop').fadeIn('slow');
},function(e){
   e.preventDefault();
   $('.product_pop').fadeOut('fast');
});